...
namespace FDK.メディア
{
    ...
    {
        // singleton
        public static グラフィックデバイス Instance
        {
            get;
            protected set;
        } = null;
        ...
        //----------------
        public Size2F 設計画面サイズ
        {
            get;
            protected set;
        } = Size2F.Empty;
        public Size2F 物理画面サイズ
        {
            get;
            protected set;
        } = Size2F.Empty;
        // ↑ int より float での利用が多いので、Size/Size2 ではなく Size2F とする。（int 同士ということを忘れて、割り算しておかしくなるケースも多発したので。）
        ...
        public void D2DBatchDraw( SharpDX.Direct2D1.DeviceContext1 dc, Action 描画処理 )
        {
            ...
            // もしリストに登録されていたら、この dc は他の誰かが BeginDraw して EndDraw してない状態なので、これらを呼び出さない。
            bool BeginとEndを行う = !( this._Draw中のレンダーターゲットリスト.Contains( dc ) );
            try
            {
                if( BeginとEndを行う )
                {
                    this._Draw中のレンダーターゲットリスト.Add( dc );     // Begin でリストに追加。
                    dc.BeginDraw();
                }
            ...
            finally
            {
                ...
                {
                    dc.EndDraw();
                    this._Draw中のレンダーターゲットリスト.Remove( dc );  // End でリストから削除。
                }
            }
        }
        ...
        public void D2DBatchDraw( SharpDX.Direct2D1.RenderTarget rt, Action 描画処理 )
        {
            ...
            // もしリストに登録されていたら、この rt は他の誰かが BeginDraw して EndDraw してない状態なので、これらを呼び出さない。
            bool BeginとEndを行う = !( this._Draw中のレンダーターゲットリスト.Contains( rt ) );
            try
            {
                if( BeginとEndを行う )
                {
                    this._Draw中のレンダーターゲットリスト.Add( rt );     // Begin 時にリストに追加。
                    rt.BeginDraw();
                }
            ...
            finally
            {
                ...
                {
                    rt.EndDraw();
                    this._Draw中のレンダーターゲットリスト.Remove( rt );  // End 時にリストから削除。
                }
            }
        }
        ...
        public void サイズを変更する( Size newSize )
        {
            ...
            this.物理画面サイズ = new Size2F( newSize.Width, newSize.Height ); // 更新
        ...
        private void _スワップチェーンを作成する()
        {
            ...
            {
                this.SwapChain = new SharpDX.DXGI.SwapChain1( dxgiFactory, this.D3DDevice, ref swapChainDesc );   // CreateSwapChainForComposition
