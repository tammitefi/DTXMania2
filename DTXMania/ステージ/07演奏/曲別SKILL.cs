...
namespace DTXMania.ステージ.演奏
{
    class 曲別SKILL : Activity
    {
        public 曲別SKILL()
        {
            ...
            {
                this.子Activityを追加する( this._数字画像 = new 画像フォント( @"$(System)images\パラメータ文字_大太斜.png", @"$(System)images\パラメータ文字_大太斜.yaml", 文字幅補正dpx: -6f ) );
                this.子Activityを追加する( this._ロゴ画像 = new 画像( @"$(System)images\曲別SKILLアイコン.png" ) );
            }
        }
