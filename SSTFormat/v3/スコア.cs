...
namespace SSTFormat.v3
{
    public partial class スコア
    {
        ...
        ///		プレビュー画像または動画のファイル名。
        /// </summary>
        public string プレビューファイル名 { get; set; }
        ...
        public static スコア ファイルから生成する( string スコアファイルパス )
        {
            ...
            {
                case ".sstf":
                    return SSTF.ファイルから生成する( スコアファイルパス );

                case ".dtx":
                    return DTX.ファイルから生成する( スコアファイルパス );

                default:
                    throw new Exception( "未対応のスコアファイルです。" );
            }
        }
        ...
        /// <summary>
        ///		小節線・拍線の追加、発声時刻の計算などを行う。
        /// </summary>
        internal static void _後処理を行う( スコア score )
        {
            // 小節線、拍線

            #region " 拍線を追加する。（小節線を先に追加すると小節が１つ増えてしまうので、先に拍線から追加する。）"
            //-----------------
            int 最大小節番号 = score.最大小節番号を返す();      // 最大小節番号はチップ数に依存して変化するので、for 文には組み込まないこと。

            for( int i = 0; i <= 最大小節番号; i++ )
            {
                double 小節長倍率 = score.小節長倍率を取得する( i );
            ...
            foreach( チップ chip in score.チップリスト )
            {
                ...
                double 時刻sec = ( チップが存在する小節の先頭時刻ms + ( _BPM初期値固定での1小節4拍の時間ms * チップが存在する小節の小節長倍率 * chip.小節内位置 ) / chip.小節解像度 ) / 1000.0;
