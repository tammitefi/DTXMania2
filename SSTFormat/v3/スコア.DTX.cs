...
namespace SSTFormat.v3
{
    public partial class スコア
    {
        ...
        {
            /// <summary>
            ///		ファイルからDTXデータを読み込み、スコアを生成して返す。
            ///		読み込みに失敗した場合は、何らかの例外を発出する。
            /// </summary>
            public static スコア ファイルから生成する( string DTXファイルパス, bool ヘッダだけ = false )
            {
                string 全入力文字列 = null;

                // ファイルの内容を一気読み。
                using( var sr = new StreamReader( DTXファイルパス ) )
                    全入力文字列 = sr.ReadToEnd();

                // 読み込んだ内容でスコアを生成する。
                var score = 文字列から生成する( 全入力文字列 );

                // ファイルから読み込んだ場合のみ、このメンバが有効。
                score.譜面ファイルパス = DTXファイルパス;
            ...
            /// <summary>
            ///     DTXフォーマットのテキストデータを含んだ１つの文字列から、スコアを生成して返す。
            ///		読み込みに失敗した場合は、何らかの例外を発出する。
            /// </summary>
            public static スコア 文字列から生成する( string 全入力文字列, bool ヘッダだけ = false )
            {
                // 文字列からスコアを生成する。
                var score = _全行解析する( ref 全入力文字列, ヘッダだけ );
            ...
            private static class 現在の
            {
                ...
                public static float BASEBPM = 0.0f;
                ...
                ///		[key: 小節番号, value: 倍率]
                /// </summary>
                public static SortedDictionary<int, float> 小節長倍率マップ;

                /// <summary>
                ///		[key: zz番号, value: BPM]
                /// </summary>
                public static Dictionary<int, float> BPM定義マップ;
            ...
            internal static bool _行解析_TITLE()
            {
            ...
            internal static bool _行解析_ARTIST()
            {
            ...
            internal static bool _行解析_COMMENT()
            {
            ...
            internal static bool _行解析_BASEBPM()
            {
            ...
            internal static bool _行解析_BPMzz()
            {
            ...
            internal static bool _行解析_WAVPANzz_PANzz()
            {
                int zz位置 = 0;
            ...
            internal static bool _行解析_WAVVOLzz_VOLUMEzz()
            {
                int zz位置 = 0;
            ...
            internal static bool _行解析_PATH_WAV()
            {
            ...
            internal static bool _行解析_WAVzz()
            {
                ...
                if( 現在の.スコア.WAVリスト.ContainsKey( zz ) )
                    現在の.スコア.WAVリスト.Remove( zz );    // 古いほうは削除。

                // ここでは、PATH_WAV はまだ反映しない。
                現在の.スコア.WAVリスト.Add( zz, (現在の.パラメータ, true) );
                return true;
            }
            internal static bool _行解析_AVIzz()
            {
                ...
                // すでに存在する zz 番号の場合は、新しいほうのみを有効とする。
                if( 現在の.スコア.AVIリスト.ContainsKey( zz ) )
                    現在の.スコア.AVIリスト.Remove( zz );    // 古いほうは削除。

                // ここでは、PATH_WAV はまだ反映しない。
                現在の.スコア.AVIリスト.Add( zz, 現在の.パラメータ );
                return true;
            }
            internal static bool _行解析_DLEVEL()
            {
                ...
                {
                    現在の.スコア.難易度 = Math.Min( Math.Max( level, 0 ), 99 ) / 10.0;     // 0～99 → 0.0 ～ 9.9
                    return true;
                }
                else
                {
            ...
            internal static bool _行解析_PREIMAGE()
            {
            ...
            internal static bool _行解析_オブジェクト記述()
            {
                ...
                if( 0x02 == 現在の.チャンネル番号 )
                {
                    ...
                    {
                        Debug.WriteLine( $"ch02 のパラメータ（小節長倍率）に指定された実数の解析に失敗しました。この行を無視します。[{現在の.行番号}行目]" );
                        return false;
                    }
                    else if( 0.0 >= 小節長倍率 )
                    {
                        Debug.WriteLine( $"ch02 のパラメータ（小数長倍率）に 0 または負数を指定することはできません。この行を無視します。[{現在の.行番号}行目]" );
                        return false;
                    }
                ...
                for( 現在の.オブジェクト番号 = 0; 現在の.オブジェクト番号 < 現在の.オブジェクト総数; 現在の.オブジェクト番号++ )
                {
                    ...
                    if( 0 > オブジェクト値 )
                    {
                        Debug.WriteLine( $"オブジェクトの記述に不正があります。この行を無視します。[{現在の.行番号}行目]" );
                        return false;
                    }
                    //----------------
                    #endregion
